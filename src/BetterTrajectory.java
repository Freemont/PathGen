...
public class BetterTrajectory {
        ...
        for (int seg = 0; seg < Segments.size(); seg++) {
            ...
            while(true) {
                ...
                if (outside == 1) { //RIGHT WHEEL OUTSIDE
                    ...
                    if (RCV < maxvel) {
                        if (RCV + maxaccel*timedelta > maxvel) {
                            RCV = maxvel;
                        } else {
                            RCV += maxaccel*timedelta;
                        }
                    }
                    ...
                    if (t >= 1){
                        t -= 1;
                        break;//remove 1 if we are into the next segment
                    }
                    ...
                    if (LCV < maxvel) {
                        if (LCV + maxaccel*timedelta > maxvel) {
                            LCV = maxvel;
                        } else {
                            LCV += maxaccel*timedelta;
                        }
                    }
    ...
    public void saveLCVRCVToCSV(String filename){
        try {
            ...
            printWriter.println("LCV,RCV, time"); //CSV Header column names

            for(PathPoint point : traj) { //Writing the LCV and RCV Values as CSV Rows
                printWriter.println(point.lvel + "," + point.rvel + "," + point.time); //It is lvel and rvel because there is a velocity at that specific point, not a cumulative velocity
            }
